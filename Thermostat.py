...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ## pulses the red light when heating up
        redLight.pulse()
    ...
    def on_exit_heat(self):
        ## turns red light off when exiting heat state
        redLight.off()
    ##
    ...
    def on_enter_cool(self):
        ## pulses blue light when cooling 
        blueLight.pulse()
    ...
    def on_exit_cool(self):
        ## turns off blue light when exiting cool state
        blueLight.off()
    ...
    def on_enter_off(self):
        ## turns off both red and blue lights when in off state 
        redLight.off()
        blueLight.off()
    ...
    def processTempStateButton(self):
        ...
        ## changes from heat to cool to off with button press
        self.cycle()
    ...
    def processTempIncButton(self):
        ...
        ## on button press increasese the setPoint by 1 
        self.setPoint += 1
        self.updateLights()
    ##
    ...
    def processTempDecButton(self):
        ...
        ## on button press decreases setPoint by 1
        self.setPoint -= 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        ## pulses red light if heating and temp below serPoint
        ## stays constant red if at the setPoint or above
        ...
        ## pulses blue light if cooling and temp above setPoint
        ## stays constant blue if at the setPoint or below
    ...
    def setupSerialOutput(self):
        ##
        ## setupSerialOutput - This function creates a string with the current status 
        ## of the thermostat so it can be sent out over the serial (UART) connection.
        ##
        ## It grabs the current state (off, heat, cool), the temperature in Fahrenheit, 
        ## and the current setpoint, and puts them all together in one line separated 
        ## by commas. This is how we're sending updates to the server that’s listening.
        ##
        ## For example, the output might look like: "cool,73.5,72"
        ## which means the thermostat is in cooling mode, the temp is 73.5°F,
        ## and the user’s setpoint is 72°F.
        ##
        ## This string will be sent out every 30 seconds while the program is running.
        ##
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Display the current date and time on the first line of the LCD
            lcd_line_1 = current_time.strftime("%b %d %H:%M:%S\n")
    
            ## Setup Display Line 2
            if(altCounter < 6):
                ##  Display the current temperature in Fahrenheit on line 2
                lcd_line_2 = f"Temp: {self.getFahrenheit():.1f}F"
                altCounter = altCounter + 1

            else:
                ##  Display the current state (off/heat/cool) and thermostat setpoint
            ...
            if((counter % 30) == 0):
                ##  Send the thermostat’s current state, temperature, and setpoint to the server
...
## Assigns the function to cycle the thermostat state when the 
## green button (GPIO 24) is pressed. This triggers the state 
## machine to move between off, heat, and cool states.
...
##
redButton = Button(25)
## Assigns the function to increase the temperature setpoint 
## when the red button (GPIO 25) is pressed. This raises the 
## target temperature by one degree Fahrenheit.
...
## Assigns the function to decrease the temperature setpoint 
## when the blue button (GPIO 12) is pressed. This lowers the 
## target temperature by one degree Fahrenheit.
