...
class CWMachine(StateMachine):
    ...
    def on_enter_dot(self):
        ...
        self.redLight.on(); sleep(0.5); self.redLight.off()
    ...
    def toggleMessage(self):
        ...
        if DEBUG:
            ...
            # Pause briefly so the message change registers before the next loop
            sleep(1)
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        if x == ".":
                            self.doDot()      # to dot
                            self.doDot()      # back to off
                        elif x == "-":
                            self.doDash()     # to dash
                            self.doDash()     # back to off

                        ## Dot/Dash pause if there's another symbol
                        if morseCounter < lenMorse:
                            self.doDDP()
                            self.doDDP()
                            morseCounter += 1

                    ## Letter pause if there's another letter
                    if wordCounter < lenWord:
                        self.doLP()
                        self.doLP()
                        wordCounter += 1

                ## Word pause if there's another word
                if wordsCounter < lenWords:
                    self.doWP()
                    self.doWP()
                    wordsCounter += 1

            ## Clear display after full message
